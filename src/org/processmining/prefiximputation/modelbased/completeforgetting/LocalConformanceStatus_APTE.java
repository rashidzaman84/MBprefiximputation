 ...
 */
public class LocalConformanceStatus_APTE {
	...
	protected LocalModelStructure_APTE lms;
	...
	//protected OnlineConformanceChecker1_APTE OCC1;
	public PrefixAlignmentBasedOCC_APTE OCC2;
	...
	public LocalConformanceStatus_APTE (LocalModelStructure_APTE lms) {
		this.lms = lms;
		//this.OCC2 = new OnlineConformanceChecker2(this.lms);
		//this.lastUpdate = new Date();		
	}

	public LocalConformanceStatus_APTE (LocalModelStructure_APTE lms2, String caseId ) {
		this.lms = lms2;
		//this.ccAlgoChoice = lms.ccAlgoChoice;
		//this.OCC2 = new OnlineConformanceChecker2(this.lms);		
		//this.imputationRevisitWindowSize = lms.imputationRevisitWindowSize;
		this.imputationRevisitSelected = lms2.imputationRevisitWindowSize==0?false:true;
		this.caseId = caseId;
	...
	public LocalConformanceStatus_APTE (LocalModelStructure_APTE lms2, String caseId, PrefixAlignmentBasedOCC_APTE OCC2 ) {
		this.lms = lms2;
		//this.ccAlgoChoice = lms.ccAlgoChoice;
		//this.OCC2 = new OnlineConformanceChecker2(this.lms);		
		//this.imputationRevisitWindowSize = lms.imputationRevisitWindowSize;
		...
		if(isNew) {			
			...
			if(lms.isInProcessModelAlphabet(newEventName) && !lms.isCaseStartingEvent(newEventName)){							
				if(lms.labelsToModelElementsMap.get(newEventName).size() == 1) {
					...
					traceModelAlphabet = PrefixImputationStatic_APTE.imputePrefix(lms, newEventName);
					...
					for(Transition transition: lms.labelsToModelElementsMap.get(newEventName)){
						...
						temp.addAll(PrefixImputationStatic_APTE.imputePrefix(lms, transition.getLabel()));
...
//
//				}				
				OCC2 = new PrefixAlignmentBasedOCC_APTE(this.lms);	//1			
...
//				return last;
				...
				OCC2 = new PrefixAlignmentBasedOCC_APTE(this.lms);  //2
				...
				for(Transition transition : mappedTransitions) {
					...
					if(deterministicPlaces.containsAll(inputPlaces)){             //we check if the transition corresponding to the new event is DIRECTLY enabled in the deterministic places of the current marking
						...
						if(!temp.isEmpty()) {
							...
							traceModelAlphabet.add(newEventName);
							traceStreamAlphabet = transformAlphabet(traceModelAlphabet);
							OCC2 = new PrefixAlignmentBasedOCC_APTE(this.lms);  //3
							batchCCPrefAlign(traceStreamAlphabet);
...
//					traceModelAlphabet.add(newEventName);
//					double traceCost = OCC2.replayer.getDataStore().get(caseId).getCost();
//					OCC2 = new PrefixAlignmentBasedOCC_APTE(this.lms);
//					traceStreamAlphabet = transformAlphabet(traceModelAlphabet);
//					batchCCPrefAlign(traceStreamAlphabet);
//					refreshUpdateTime();
...
//		for (int i = 0; i < xtrace.size(); i++) {
//			if(i==0) {
//				this.OCC1 = new OnlineConformanceChecker1_APTE();
