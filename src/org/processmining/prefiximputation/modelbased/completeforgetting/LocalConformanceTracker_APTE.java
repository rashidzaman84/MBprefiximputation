 ...
 */
public class LocalConformanceTracker_APTE extends HashMap<String, LocalConformanceStatus_APTE> {
...
//	public int noOfForgottenCases = 0;
	...
	public LocalConformanceTracker_APTE(LocalModelStructure_APTE lms, int maxCasesToStore) {
		//this.caseIdHistory = new HashMap<String, Integer>();
		this.lms = lms;
		this.maxCasesToStore = maxCasesToStore;
		//OCC2 = new PrefixAlignmentBasedOCC_APTE(lms);
	...
	public OnlineConformanceScore replayEvent(String caseId, String newEventName) {
		...
		if (containsKey(caseId)) {
			...
			if (size() >= maxCasesToStore) {
				...
				ForgettingCases_APTE forgettingCases = new ForgettingCases_APTE(lms, this);               //We can calculate a deletion candidacy score for relevant cases on arrival of every new event but that will
...
//					partialAlignmentRecord.get(caseToForget).add(partialAlignment);
//				}
				//System.out.println(OCC2.replayer.getDataStore().size());
				this.remove(caseToForget); //Removed the LocalConformanceTracker HashMap entry corresponding to the toRemove case ID
				//OCC2.replayer.getDataStore().remove(caseToForget); 
				
			}			
			
			// now we can perform the replay
			//LocalConformanceStatus_APTE lcs = new LocalConformanceStatus_APTE(lms, caseId, OCC2);
			LocalConformanceStatus_APTE lcs = new LocalConformanceStatus_APTE(lms, caseId);
